...
class TLSSocket(object):
    ...
    def _raw_write(self):
        ...
        while len(to_write):
            ...
            try:
                ...
                # Handle ECONNRESET, EPIPE, EBADF
                if e.errno == 104 or e.errno == 54 or e.errno == 32 or e.errno == 9:
    ...
    def _read_certificates(self):
        ...
        if libcrypto_version_info < (1, 1):
            number_certs = libcrypto.sk_num(stack_pointer)
        else:
            number_certs = libcrypto.OPENSSL_sk_num(stack_pointer)

        self._intermediates = []

        for index in range(0, number_certs):
            if libcrypto_version_info < (1, 1):
                x509_ = libcrypto.sk_value(stack_pointer, index)
            else:
                x509_ = libcrypto.OPENSSL_sk_value(stack_pointer, index)
