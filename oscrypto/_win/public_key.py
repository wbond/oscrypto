...
def _load_key(key_object, algo, container):
    ...
    try:
        if algo == 'ecdsa':
            ...
            if curve_type != 'named':
                raise PrivateKeyError('Windows only supports ECDSA keys using named curves')
            if curve_name not in ('secp256r1', 'secp384r1', 'secp521r1'):
                raise PrivateKeyError('Windows only supports ECDSA keys using the named curves secp256r1, secp384r1 and secp521r1')
